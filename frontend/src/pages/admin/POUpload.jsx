        ...
        <table style={{ width: "100%", fontSize: "0.85rem", borderCollapse: "collapse" }}>
          ...
          <tfoot>
            ...
            <tr style={{ borderTop: "2px solid var(--border-medium, #e2e8f0)", background: "var(--bg, #f6f8fb)" }}>
              ...
              <td style={{ textAlign: "right", padding: "10px 14px", fontWeight: 700, color: "#b45309" }}>{fmt.format(logs.reduce((s, l) => s + (Number(l.lines_skipped) || 0), 0))}</td>{/* Skipped */}
              <td style={{ textAlign: "right", padding: "10px 14px", fontWeight: 700, color: "#475569" }}>{fmt.format(logs.reduce((s, l) => s + (Number(l.lines_skipped_closed) || 0), 0))}</td>{/* Closed */}
              <td style={{ textAlign: "right", padding: "10px 14px", fontWeight: 700, color: "#b91c1c" }}>{fmt.format(logs.reduce((s, l) => s + (Number(l.lines_skipped_cancelled) || 0), 0))}</td>{/* Cancelled */}
