...
/**
 * The five canned PIC reports — pill bar, per-report filters, table, totals.
 *
 * Lifted out of pic/PICReports.jsx so the PM's PIC Overview runs the SAME
 * reports rather than a second implementation free to drift from it. The
 * export / CSV / refresh buttons moved into the card header, because the panel
 * is no longer the whole page on either host and a page header it does not own
 * is not somewhere it can put them.
 */
const REPORTS = [
  {
    id: "pipeline",
...
/**
 * `project` / `range` may be supplied by the host page (PIC Overview drives
 * both from one filter bar). When they are, the panel uses them and drops its
 * own copies of those two controls rather than showing a second, conflicting
 * set. Owner stays panel-local — only the Aging report has one.
 */
export default function PicReportsPanel({ project: projectProp, range: rangeProp }) {
  const controlled = projectProp !== undefined || rangeProp !== undefined;
  const [kind, setKind] = useState("pipeline");
  // Deliberately NOT defaulted to a rolling window like "last 3 months":
  // closures lag, and a clock-based default renders an empty report whenever
  // the most recent close is older than the window. The row limit below is
  // what keeps this report cheap; the range is the user's to set.
  const [ownRange, setOwnRange] = useState({ from: "", to: "" });
  const [ownProject, setOwnProject] = useState([]);
  const [owner, setOwner] = useState("");
  const range = rangeProp !== undefined ? rangeProp : ownRange;
  const project = projectProp !== undefined ? projectProp : ownProject;
  // Local, not the page-global row limit: the panel is embedded in two
  // different pages and shouldn't fight whatever table limit they set.
  ...
  }, [kind, range.from, range.to, JSON.stringify(project), owner, rowLimit]);

  const shown = data?.rows?.length || 0;
  // `total` is the backend's true match count, independent of the limit. Only
  // the bounded reports send it; the rest are naturally small and send none,
  // in which case what is on screen IS everything.
        ...
        <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, overflow: "hidden", boxShadow: "0 1px 2px rgba(15,23,42,0.04)" }}>
          ...
          <div style={{ padding: "14px 18px", borderBottom: "1px solid #e2e8f0" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div style={{ marginLeft: "auto", fontSize: "0.78rem", color: truncated ? "#c2410c" : "#94a3b8", whiteSpace: "nowrap", fontWeight: truncated ? 700 : 400 }}>
                {shown
                  ? (truncated
                      ? `${fmtInt.format(shown)} of ${fmtInt.format(total)} rows`
                      : `${fmtInt.format(shown)} row${shown !== 1 ? "s" : ""}`)
                  : ""}
              </div>
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 14 }}>
              {showDate && !controlled && (
                <DateRangePicker value={range} onChange={({ from, to }) => setOwnRange({ from, to })} />
              )}
              {showProject && !controlled && (
                <SearchableSelect multi value={project} onChange={setOwnProject} options={projectOptions} placeholder="All Projects" minWidth={180} />
              )}
              {showOwner && (
              ...
              {controlled && (showDate || showProject) && (
                <span style={{ fontSize: "0.76rem", color: "#94a3b8", fontStyle: "italic" }}>
                  Using the page filters above
                  {showDate && showProject ? " (date range + project)" : showDate ? " (date range)" : " (project)"}.
                </span>
              )}
          ...
          <DataTableWrapper
            loadedCount={loading ? null : shown}
                  ...
                  <tfoot>
                    <tr style={{ background: "#f1f5f9", fontWeight: 700 }}>
                      {(data.columns || []).map((c, idx) => {
                        ...
                        if (idx === 0) {
                          return (
                            <td key={c.key} style={{ color: "#475569", fontSize: "0.82rem" }}>
                              {/* Say what the figures actually add up — a total
                                  over 500 of 11,389 rows is not the total. */}
                              TOTAL · {truncated
                                ? `${fmtInt.format(shown)} of ${fmtInt.format(total)} rows`
                                : `${fmtInt.format(shown)} rows`}
                            </td>
                          );
                        }
