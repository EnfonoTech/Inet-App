...
export const pmApi = {
  ...
  reportPoDispatchStatus:         () => call("inet_app.api.command_center.get_po_dispatch_status_report"),
  reportPoMilestoneStatus:        () => call("inet_app.api.command_center.get_po_milestone_status_report"),
...
/**
 * IM report catalog — the IM-side twin of the report methods on pmApi above.
 *
 * Every endpoint behind these resolves the IM from the SESSION and scopes its
 * own query (inet_app/api/im_reports.py). Nothing here sends an `im`: there is
 * nothing useful an IM could put in one, and the server would ignore it
 * anyway. A `team` may be sent to narrow within the IM's own teams — the
 * server intersects it with that IM's teams before using it, so it can only
 * ever subtract rows, never add them.
 */
const imReport = (method) => (f) =>
  call(`inet_app.api.im_reports.${method}`, { filters: JSON.stringify(f || {}) });

export const imReportsApi = {
  // Who the catalog is scoped to, and the teams in that scope
  getScope:           () => call("inet_app.api.im_reports.get_im_report_scope"),
  getTeamOptions:     () => call("inet_app.api.im_reports.get_im_team_options"),
  getHuaweiOptions:   () => call("inet_app.api.im_reports.get_im_huawei_filter_options"),

  // Teams & Utilization
  reportTeamUtilization:      imReport("report_team_utilization"),
  reportMonthlyTeamDetails:   imReport("report_monthly_team_details"),
  reportTeamPVA:              imReport("report_team_pva"),

  // Performance
  reportWeeklyPerformance:    imReport("report_weekly_performance"),
  reportTopTeams:             imReport("report_top_teams"),

  // Project / Rollout & Delivery
  reportProjectPerformance:   imReport("report_project_performance"),
  reportRolloutBurnDown:      imReport("report_rollout_burn_down"),
  reportPoDispatchStatus:     imReport("report_po_dispatch_status"),
  reportPoMilestoneStatus:    imReport("report_po_milestone_status"),

  // CIAG Site Sign & Verify
  reportSiteSignStatus:       imReport("report_site_sign_status"),
  reportSiteVerifyStatus:     imReport("report_site_verify_status"),

  // Material
  reportBillWiseStatus:       imReport("report_bill_wise_status"),
  reportHuaweiOutboundAnalytics: imReport("report_huawei_outbound_analytics"),

  // Team Idle by Domain renders its own grid, so it takes named args rather
  // than the shared `filters` blob — same shape as pmApi.getTeamDomainUtilization.
  getTeamDomainUtilization: ({ month, domains, include_fridays, etag } = {}) =>
    call("inet_app.api.im_reports.report_team_idle_domain", {
      month: month || "",
      // Same empty-string-not-"[]" shape as pmApi.getTeamDomainUtilization,
      // so both sides hit _ensure_list() with exactly what it already handles.
      domains: domains?.length ? JSON.stringify(domains) : "",
      include_fridays: include_fridays ? 1 : 0,
      etag: etag || "",
    }),
};
