...
def list_pic_rows(filters=None, limit=500, portal_filters=None, with_team_type=0, stage=None, _options=None, _summary=None):
    ...
    col_filter_map = {
        ...
        "project_domain": "COALESCE(NULLIF(pd.project_domain,''), proj.project_domain, '')",
    ...
    if search:
        concat_parts_pic = [
            ...
            "COALESCE(NULLIF(pd.project_domain,''), proj.project_domain, ''),",
    ...
    SELECT  /* {limit_page_length} = 0 → unlimited; with_team_type={int(with_team_type)} */
      ...
      pd.line_amount,
      pd.tax_rate,
      COALESCE(NULLIF(pd.project_domain,''), proj.project_domain, '') AS project_domain,
      pd.project_code,
