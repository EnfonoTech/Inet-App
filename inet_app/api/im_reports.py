...
def _safe_teams(f, allowed):
    ...
    an empty list, and an empty list is passed through as "no team narrowing"
    only when the browser sent nothing at all.
    """
    want = f.get("team") or []
...
def report_team_utilization(filters=None):
    ...
    # one grew a message — it caps its own row count and says so when the
    # oldest days were left out — so unpack by position, not to a fixed pair.
    # The message has to reach the page: a truncation warning nothing displays
    # is the silent-wrong-total the cap exists to prevent.
    result = execute({
...
def get_im_huawei_filter_options():
    """Project / Domain options for the IM's Huawei Outbound Analytics filter.

    The PM's equivalent lists every project in Huawei Outbound Plan. Here the
    list is cut to the IM's own projects, so the dropdown cannot offer a
    project that would only ever come back empty once the report's own scope
    is applied.
    """
    _im, im_ids, _teams = _im_session_scope()
    _duids, projects = _im_duid_scope(im_ids)
    ...
    # Subcontractors are taken from the IM's own projects too, so the dropdown
