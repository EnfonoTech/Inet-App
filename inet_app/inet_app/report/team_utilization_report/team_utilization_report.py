...
def execute(filters=None):
    ...
    data = frappe.db.sql(
        ...
        SELECT
            ...
            -- COUNT(DISTINCT ... rp.name), not SUM(CASE ... THEN 1): the LEFT
            -- JOIN to Daily Execution multiplies rows, so a team with two
        ...
        LEFT JOIN `tabDaily Execution` de ON de.rollout_plan = rp.name
            -- IFNULL, not `!=`: in SQL `NULL != 'Cancelled'` evaluates to NULL
            -- rather than true, so an execution row with no status set was
            -- dropped from achieved qty/amount instead of counted.
            AND IFNULL(de.execution_status, '') <> 'Cancelled'
            -- Achieved means DELIVERED, measured on the same basis as
            -- Completed Activities: the plan is closed. Before this, any
            -- non-cancelled execution contributed, so a team could show 0
            -- completed activities and 0 achievement beside a real achieved
            -- amount — work recorded against a plan still In Execution or
            -- Planning with Issue. Every column now answers one question,
            -- so there is no contradiction left on the row to explain.
